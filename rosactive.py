...
def init_file_structure():
    print("This appears to be your first time running Rosactive. First time setup will start now.")
    if 'ROS_DISTRO' in environ.keys():
        print("ERROR: You appear to have some ROS content still sourced in your shell configuration. Please only use rosactive in an environment where no ROS content has been sourced (so that rosactive can do all the management)")
        sys.exit()
